  ...
  $(function() {
    ...
    yearBtn.on('click', function(e) {
      ...
      $.ajax({
        method: 'GET',
        url:
          window.isf_vars.rest_url +
          'wp/v2/isf_event?filter[meta_key]=event_date&filter[meta_value]=' +
          year +
        ...
        $.each(data, function(key, value) {
          ...
          if (month === 0) {
            january.append(
              '<article class="timeline-single-post"><img src=' +
                value.acf.event_image +
                '><p>' +
    ...
    yearBtnMobile.on('click', function(e) {
      ...
      $.ajax({
        ...
        $.each(data, function(key, value) {
          ...
          timelineMobile.append(
            '<div class="timeline-post-title-mobile"><div class="timeline-bullet-mobile"></div><h3 class="month-font-mobile">' +
              month +
              '</h3></div><article class="timeline-single-post timeline-post-mobile"><img src=' +
              value.acf.event_image +
              '><p>' +
              value.title.rendered +
              '</p></article>'
